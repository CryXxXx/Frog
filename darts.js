...
function info(tg_id) {
    fetch('https://36e0fd5e46a0cf.lhr.life/re_info', {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        let newValue = data.balance; // Предполагается, что это число с точкой
...
function getCurrentBalance() {
    ...
    return parseFloat(userBalanceElement.textContent.replace(',', '.')) || 0;
}

// Функция для обновления баланса
function updateBalance(newBalance) {
    const userBalanceElement = document.querySelector('.user-balance');
    userBalanceElement.textContent = newBalance.toFixed(2).replace('.', ',');
}
    ...
    if (betButtonSup.style.opacity === '1') {
        ...
        anime({
            ...
            complete: function() {
                ...
                fetch('https://36e0fd5e46a0cf.lhr.life/darts', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
