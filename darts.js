...
function info(tg_id) {
    fetch('https://fcaed1e3d53a0d.lhr.life/re_info', {
        method: 'POST',
        headers: {
...
function updateBalance(newValue) {
    ...
    const formattedValue = newValue.replace('.', ','); // Заменяем точку на запятую

    // Просто подставляем новое значение в элемент
    userBalanceElement.innerText = formattedValue;
}
...
function handleBetButtonClick() {
    ...
    if (betButtonSup.style.opacity === '1') {
        ...
        const betAmount = parseFloat(betNumber);
        ...
        anime({
            ...
            complete: function() {
                ...
                fetch('https://fcaed1e3d53a0d.lhr.life/darts', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
