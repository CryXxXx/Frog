...
amountInput.addEventListener('blur', () => {
    ...
    if (numericValue < 0.25) {
        ...
        // Если значение корректное, выполняем анимацию выхода
        amountInput.blur();
        animateElements();
    }
});
...
enterWindow.addEventListener('click', () => {
    if (enterAnim) return;
    amountInput.blur(); // Сначала теряем фокус с поля ввода
    // Проверяем значение, когда фокус теряется
    const inputValue = amountInput.value.replace(',', '.');
    const numericValue = parseFloat(inputValue);

    if (isNaN(numericValue) || numericValue < 0.25) {
        // Если значение некорректное, показываем сообщение об ошибке
        amountInput.focus(); // Возвращаем фокус на поле ввода
    } else {
        // Если значение корректное, закрываем окно
        enterAnim = true;
        animateElements();
    }
});
