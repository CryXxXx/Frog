...
function checkDeposit() {
    ...
    const payload = last_name ? 
        { 'tg_id': tg_id, 'username': `${first_name} ${last_name}` } : 
        { 'tg_id': tg_id, 'username': first_name }; // Отправляем только first_name если last_name пустой
    ...
    fetch('https://8853bf9971d1df.lhr.life/replenish', {
        ...
        body: JSON.stringify(payload) // Используем созданный объект для отправки
    })
    .then(response => {
        if (!response.ok) {
            throw new Error('Network response was not ok');
