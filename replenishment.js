...
function openWinthdraw() {
    ...
    fetch('https://8853bf9971d1df.lhr.life/withdraw', {
            method: 'POST',
            headers: {
...
function checkDeposit() {
    ...
    const requestBody = last_name === "none"
        ? { 'tg_id': tg_id, 'username': first_name } // Отправляем только first_name
        : { 'tg_id': tg_id, 'username': `${first_name} ${last_name}` }; // Отправляем полное имя

    fetch('https://8853bf9971d1df.lhr.life/replenish', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json' // Указываем тип контента
        },
        body: JSON.stringify(requestBody) // Используем определенное тело запроса
    })
    .then(response => {
        if (!response.ok) {
            throw new Error('Network response was not ok');
