...
function animateElements() {
    anime({
        ...
        complete: () => {
            ...
            enterAnim = false;
            amountInput.value = '';
            amountText.textContent = 'Введите Сумму';
        }
    });
}
...
amountInput.addEventListener('input', () => {
    ...
    amountText.textContent = inputValue || 'Введите Сумму';
});
...
amountInput.addEventListener('blur', () => {
    ...
    if (numericValue < 0.25) {
        ...
        if (amountText.textContent === 'Введите Сумму') {
            animateElements();
            return;
        }
...
const apngPaths = [
    'assets/darts-apng-1.png',
    'assets/darts-apng-2.png',
    'assets/darts-apng-3.png',
    'assets/darts-apng-4.png',
    'assets/darts-apng-5.png',
    'assets/darts-apng-6.png',
];

let apngs = [];

function preloadAPNGs() {
    const promises = apngPaths.map(src => {
        return new Promise((resolve, reject) => {
            const apng = new Image();
            apng.src = src;
            apng.onload = () => resolve(apng);
            apng.onerror = () => reject(new Error(`Failed to load image: ${src}`));
        });
    });

    return Promise.all(promises).then(images => {
        apngs = images; // Сохраняем загруженные APNG
    });
}
...
function info(tg_id) {
    fetch('https://08855a33201df2.lhr.life/re_info', {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        ...
        if (hiderElement && apngs.length > 0) {
            hiderElement.remove();
        }
    })
    .catch((error) => {
...
function handleBetButtonClick() {
    ...
    if (betButtonSup.style.opacity === '1') {
        ...
        const betAmount = parseFloat(betNumberElement.textContent.replace(',', '.'));
        ...
        anime({
            ...
            complete: function() {
                ...
                fetch('https://08855a33201df2.lhr.life/darts', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
...
function handleServerResponse(result) {
    ...
    const apngPath = `assets/darts-apng-${randomValue}.png`;
    const apngInstance = new APNG(apngPath);
    ...
    anime({
        ...
        complete: () => {
            ...
            apgCont.innerHTML = '';
            const canvas = apngInstance.getCanvas();
            apgCont.appendChild(canvas);
            apngInstance.play();
